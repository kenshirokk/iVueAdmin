        ...
        <v-ons-row>
          <v-ons-col width="75%" class="app_list_row">
            ...
            <v-ons-row vertical-align="center">
              ...
              <v-ons-col width="60%">{{td.nickname}}</v-ons-col>
            </v-ons-row>
            <v-ons-row vertical-align="center">
              <v-ons-col width="40%">返利金额：</v-ons-col>
              <v-ons-col width="60%">{{td.backMoney}}</v-ons-col>
            </v-ons-row>
            <v-ons-row vertical-align="center">
            ...
            <v-ons-row vertical-align="center">
              ...
              <v-ons-col width="60%">{{$format.dateFormat(td.payDatetime)}}</v-ons-col>
            </v-ons-row>
          </v-ons-col>
        </v-ons-row>
...
<script>
  import { getPerformanceVODetail} from '@/api/report'
    ...
    export default {
        ...
        methods: {
          getList(back) {
            this.loading = true;
            getPerformanceVODetail(this.pageNum,this.pageSize,this.year,this.month,this.searchTemp.userId).then(response => {
              this.loading = false;
